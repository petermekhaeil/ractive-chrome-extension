(function () {
  ...
  */
  ...
  if (hasConsole) {
    ...
    welcome = function () {
      if (Ractive$1.WELCOME_MESSAGE === false) {
        welcome = noop;
        return;
      }
      var message = 'WELCOME_MESSAGE' in Ractive$1 ? Ractive$1.WELCOME_MESSAGE : welcomeMessage;
  ...
  defineProperties(Ractive$1, {
    ...
    styleGet: { configurable: true, value: styleGet.bind(Ractive$1) },
    styleSet: { configurable: true, value: setCSSData.bind(Ractive$1) },
    addCSS: { configurable: false, value: addStyle.bind(Ractive$1) },
    hasCSS: { configurable: false, value: hasStyle.bind(Ractive$1) }
  });
  ...
  if (win && !win.Ractive) {
    ...
    /* istanbul ignore next */
    if (~opts$1.indexOf('ForceGlobal')) { win.Ractive = Ractive$1; }
  } else if (win) {
    warn("Ractive already appears to be loaded while loading 1.3.3.");
  }

  assign(Ractive$1.prototype, proto$9, defaults);
  ...
  shared.defaults = Ractive$1.defaults;
  shared.Ractive = Ractive$1;
  ...
  defineProperty(Ractive$1, '_cssModel', {
    configurable: true,
    value: new CSSModel(Ractive$1)
  });

  defineProperty(Ractive$1.prototype, 'rendered', {
    get: function get() {
  ...
  const win$1 = typeof window !== 'undefined' ? window : null;

  function globalRegister(name, registry, constructor) {
    if (win$1 && win$1.Ractive && typeof win$1.Ractive[registry] === 'object') {
  ...
  function isArray$1(it) { return !!it && Object.prototype.toString.call(it) === '[object Array]'; }
  function getType$1(it) {
    ...
    else if (typeof it === 'object' && isArray$1(it)) return 'array';
  ...
  class JSONEditor extends Ractive$1 {
    constructor(opts) { super(opts); }

    renameKey(path, name) {
      const then = path;
      const value = this.get(then);
      path = Ractive$1.splitKeypath(path);
      const thenKey = path.pop();
      const base = this.get(join(path));
      path.push(name);
      const now = join(path);
      const edit = 'keys.' + Ractive$1.escapeKey(then);
    ...
    addKey(path) {
      const namePath = `values.${Ractive$1.escapeKey(path)}.name`;
    ...
    startEdit(path, node) {
      ...
      path = `edits.${Ractive$1.escapeKey(path)}`;
    ...
    stopEdit(path, key) {
      this.toggle(`${key || 'edits'}.${Ractive$1.escapeKey(path)}`);
      return false;
    }

    changeType(path) {
      let val = getType$1(this.get(path));
